...
RE_PATH_REF = re.compile(
    r"(?<![A-Za-z0-9_/])"
    # 선두: ../ 또는 ./ 세그먼트 1개 이상(중첩 허용), 또는 word/ 디렉터리.
    # (이전엔 `\./` 만 허용 → `../../x.md` 가 `./../x.md` 로 절단돼 오판)
    r"((?:(?:\.{1,2}/)+|[A-Za-z0-9_]+/)[A-Za-z0-9_./-]+"
    # 확장자: 접두가 겹치는 것(json>js, jsx>js, tsx>ts)을 먼저 두고,
    # 뒤에 영숫자가 오면 매치 안 함 → `EXAMPLE.json` 이 `.js` 로 절단되지 않음.
    r"\.(?:tsx|ts|jsx|json|js|py|md|sql|yaml|yml|toml|html|css|sh|go|rs|java|kt|rb|php))"
    r"(?![A-Za-z0-9])"
)
...
def score_f(modules: list[Module], repo: Path) -> CategoryScore:
    ...
    husky_hook = (repo / ".husky" / "pre-commit").exists() or (repo / ".husky" / "pre-push").exists()
    # git hook(husky)뿐 아니라 .claude PostToolUse hook이 경로를 검증해도 인정한다.
    claude_hooks_dir = repo / ".claude" / "hooks"
    claude_hook = claude_hooks_dir.is_dir() and any(
        re.search(r"context.?path|check[-_]?context|check:docs", read_text(h), re.IGNORECASE)
        for h in claude_hooks_dir.glob("*.sh")
    )
    hook_validates_paths = husky_hook or claude_hook
    ...
    if not hook_validates_paths:
        findings.append("context path를 검증하는 hook 없음 (husky pre-commit/pre-push 또는 .claude PostToolUse)")
...
def main() -> int:
    # Windows consoles default to cp949/cp1252 and choke on the report's
    # box-drawing / ⚠ glyphs. Force UTF-8 so the markdown summary prints
    # regardless of the host locale; harmless on POSIX where it's already UTF-8.
